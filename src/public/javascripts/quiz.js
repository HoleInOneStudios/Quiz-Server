...
function update() {
  QUIZ_SCORE_ELEMENT.innerText = `${parseInt(
  ...
  ANSWER_ELEMENTS.forEach((answer, index) => {
    ...
    answer.addEventListener("click", (event) => {
      if (SHEET_DATA[CURRENT_QUESTION]["answered"] != true) {
        ...
        SHEET_DATA[CURRENT_QUESTION]["selected"] = parseInt(
          answer.getAttribute("answer")
        );

        if (
          SHEET_DATA[CURRENT_QUESTION].selected ==
          SHEET_DATA[CURRENT_QUESTION].Correct
        ) {
          SCORE++;
        }
        ...
        console.log(
          "already answered: " + SHEET_DATA[CURRENT_QUESTION]["selected"]
        );
      }
      update();
    });
...
function updateAnswers() {
  if (SHEET_DATA[CURRENT_QUESTION]["answered"]) {
    ANSWER_ELEMENTS[SHEET_DATA[CURRENT_QUESTION].Correct - 1].classList.add(
      "correct"
    );
    if (
      SHEET_DATA[CURRENT_QUESTION].selected ==
      ...
      ANSWER_ELEMENTS[SHEET_DATA[CURRENT_QUESTION].selected - 1].classList.add(
        "incorrect"
      );
    }
  }
